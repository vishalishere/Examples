...
This is an update which integrates Carfax VIN Lookup. It uses the C# HunterNet.dll from the C++ GUI interface to put
the VIN next to the state and plate. 

Added Files:

HunterNet.tlb-
This is the .dll of HunterNet, but has been sent through RegAsm.

VINRetriever.h-
A class which calls the HunterNet.tlb to get the VIN. It must be sent
a plate and state. The username and password were set as constants in
the class, and if it comes to a point where username and password are
legitimate, these would need to be added to the constructor.
